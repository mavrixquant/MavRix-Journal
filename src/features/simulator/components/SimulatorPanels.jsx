// src/features/simulator/components/SimulatorPanels.jsx
import {
  FaInfoCircle, FaCheckCircle, FaExclamationTriangle, FaShieldAlt,
  FaBalanceScale, FaArrowDown, FaArrowUp, FaPercent,
  FaDice, FaLayerGroup, FaRandom,
} from 'react-icons/fa';
...
const METHOD_META = {
  permutation: {
    label: 'Permutation', icon: <FaRandom />,
    tagline: 'Sequence risk · same trades, different order',
    description: 'Reshuffles your exact same trades into different chronological orders. The total is invariant — only the shape of the path changes. Answers: "how much can sequencing hurt me?" A profitable-sample percentage is not informative here because every run has the same total P&L.',
    profitMetricMeaningful: false,
  },
  bootstrap: {
    label: 'Bootstrap', icon: <FaDice />,
  ...
  block: {
    label: 'Block Bootstrap', icon: <FaLayerGroup />,
...
const PAN_CSS = `
  .sim-panels {
    ...
    --win: #22c55e; --win-2: #86efac;
    --win-soft: rgba(34,197,94,.10); --win-soft2: rgba(34,197,94,.28);
    --loss: #ef4444; --loss-2: #fca5a5;
    --loss-soft: rgba(239,68,68,.10); --loss-soft2: rgba(239,68,68,.28);
  ...
  .sim-tab:hover { color: var(--ink-1); background: rgba(255,255,255,.04); }
  .sim-tab.is-active {
  ...
  .sim-panel:hover {
    ...
    box-shadow: 0 24px 60px -30px rgba(0,0,0,.95), 0 0 40px -18px var(--accent-soft2), inset 0 1px 0 rgba(255,255,255,.04);
  }
  .sim-panel-head {
  ...
  .sim-panel-title::before {
    ...
    box-shadow: 0 0 10px rgba(245,158,11,.6); flex-shrink: 0;
  }
  .sim-panel-note {
    font-family: 'IBM Plex Mono', ui-monospace, monospace;
  ...
  .sim-kpi-icon {
    ...
    font-size: 15px; background: rgba(255,255,255,.04);
    border: 1px solid var(--line); color: var(--ink-2);
  }
  .sim-kpi-icon.is-win {
    color: var(--win); background: var(--win-soft);
    border-color: var(--win-soft2); box-shadow: 0 0 24px -10px var(--win-soft2);
  }
  .sim-kpi-icon.is-loss {
    color: var(--loss); background: var(--loss-soft);
    border-color: var(--loss-soft2); box-shadow: 0 0 24px -10px var(--loss-soft2);
  }
  .sim-kpi-icon.is-amber {
    color: var(--accent); background: var(--accent-soft);
    border-color: var(--accent-soft2); box-shadow: 0 0 24px -10px var(--accent-soft2);
  }
  .sim-kpi-text { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
  .sim-kpi-label {
    font-family: 'IBM Plex Mono', ui-monospace, monospace;
  ...
  .sim-kpi-value {
    ...
    letter-spacing: -.01em; color: var(--ink-1); white-space: nowrap;
  }
  ...
  .sim-method-body { display: flex; flex-direction: column; gap: 6px; min-width: 0; }
  .sim-method-name { display: flex; align-items: center; gap: 10px; flex-wrap: wrap; }
  ...
  .sim-method-desc { font-size: 12.5px; line-height: 1.65; color: rgba(255,255,255,.72); }
  .sim-info-banner {
  ...
  .sim-info-banner svg { color: var(--accent); flex-shrink: 0; margin-top: 2px; }
  ...
  .sim-ul li {
    ...
    border: 1px solid var(--line-soft); align-items: flex-start;
  ...
  .sim-ul li::before {
    ...
    box-shadow: 0 0 8px rgba(245,158,11,.5); margin-top: 7px;
  }
  ...
  table.sim-table tbody td:first-child {
    text-align: left; font-weight: 600; color: var(--ink-1);
  ...
  .sim-legend-line { display: inline-block; width: 14px; height: 2px; border-radius: 99px; }
  .sim-legend-band { display: inline-block; width: 14px; height: 8px; border-radius: 3px; }
  .sim-foot {
  ...
  .sim-na-icon {
    ...
    display: inline-flex; align-items: center; justify-content: center;
    background: var(--accent-soft); border: 1px solid var(--accent-soft2);
    color: var(--accent); font-size: 20px; margin-bottom: 6px;
  }
...
/*  Overview                                                           */
/* ------------------------------------------------------------------ */
function OverviewPanel({ result, summary, isMoney, initialCapital, dollarsPerR }) {
  ...
  const ddBreachPct = summary.ddBreachPct;
  const ddThreshold = result.ddThreshold;
  const hasDdPct = initialCapital > 0 && (isMoney || dollarsPerR > 0);

  // Verdict: fact-based wording, no "statistically robust" claims.
  const verdict = (() => {
    if (showProfit) {
      if (profitPct >= 90) return { text: 'Most samples profitable',           cls: 'pos',  icon: <FaCheckCircle /> };
      if (profitPct >= 70) return { text: 'Most samples profitable',           cls: 'pos',  icon: <FaCheckCircle /> };
      if (profitPct >= 50) return { text: 'Bootstrap result is fragile',        cls: 'warn', icon: <FaExclamationTriangle /> };
      return                     { text: 'Most samples lost money',             cls: 'neg',  icon: <FaExclamationTriangle /> };
    }
    if (!hasDdPct) return       { text: 'Sequence stress test only',           cls: 'warn', icon: <FaInfoCircle /> };
    const worstPct = Math.abs(summary.maxDDPct_p5);
    if (worstPct < 10) return   { text: 'Sequence-driven DD stays shallow',    cls: 'pos',  icon: <FaCheckCircle /> };
    if (worstPct < 25) return   { text: 'Sequence-driven DD is notable',       cls: 'warn', icon: <FaInfoCircle /> };
    return                       { text: 'Sequence-driven DD is severe',        cls: 'neg',  icon: <FaExclamationTriangle /> };
  })();
      ...
      <div className="sim-kpi-grid">
        {showProfit ? (
          <KPI
            ...
            sub={`${summary.profitCount}/${result.runs} runs ended +`}
            cls={profitPct >= 50 ? 'pos' : 'neg'}
          />
        ) : (
          <KPI
            icon={<FaArrowDown />} iconCls="is-loss"
            label="Worst 5% Drawdown"
            value={fmtValue(summary.maxDD_p5, isMoney)}
            sub={hasDdPct ? `${summary.maxDDPct_p5.toFixed(1)}% of capital` : 'worst-case sequence'}
            cls="neg"
          />
        )}
        <KPI
          icon={<FaCheckCircle />}
          iconCls={summary.win_p50 >= 50 ? 'is-win' : 'is-loss'}
          label="Per-Trade WR"
          value={fmtPct(summary.win_p50)}
          sub="median across runs"
          cls={summary.win_p50 >= 50 ? 'pos' : 'neg'}
        />
        <KPI
          icon={<FaShieldAlt />}
          iconCls={ddBreachPct > 0 ? 'is-loss' : 'is-win'}
          label="DD Threshold Breach"
          value={fmtPct(ddBreachPct)}
          sub={
            ddThreshold != null
              ? `drawdown ≤ ${fmtValue(ddThreshold, isMoney)}`
              : 'no threshold'
          }
          cls={ddBreachPct > 0 ? 'neg' : 'pos'}
        />
        <KPI
          icon={<FaBalanceScale />}
        ...
        <KPI
          ...
          sub="5th pct of final P&L"
          cls={summary.p5Final >= 0 ? 'pos' : 'neg'}
        />
        <KPI
          icon={<FaArrowUp />}
          iconCls={summary.p95Final >= 0 ? 'is-win' : 'is-loss'}
          label="95th %ile Final"
          value={fmtValue(summary.p95Final, isMoney)}
          sub="95th pct of final P&L"
          cls={summary.p95Final >= 0 ? 'pos' : 'neg'}
        />
      </div>
      ...
      <Panel>
        ...
        {!showProfit && (
          <div className="sim-info-banner">
            <FaInfoCircle />
            <span>
              <b>Profitable-samples % is not meaningful in permutation mode.</b>{' '}
              Since every run uses the exact same trades, the total is identical — the
              fraction profitable is always 0% or 100%. Focus on <b>drawdown</b> and{' '}
              <b>streaks</b> instead, or switch to <b>Bootstrap</b> for a real
              profitable-sample probability.
            </span>
          </div>
        )}
        ...
        {!hasDdPct && !isMoney && (
          <div className="sim-info-banner">
            <FaInfoCircle />
            <span>
              <b>Drawdown % of capital is hidden</b> because we don't know the R→$
              conversion for this account. Configure a per-trade risk amount on the
        ...
        <ul className="sim-ul">
          {showProfit ? (
            <>
              <li>
                <span>
                  <b>{profitPct.toFixed(1)}%</b> of simulated samples ended profitable.
                  Median final: <b>{fmtValue(summary.medianFinal, isMoney)}</b>.
                </span>
              </li>
              <li>
                <span>
                  95th pct final:{' '}
                  <b className={summary.p95Final >= 0 ? 'pos' : 'neg'}>
                    {fmtValue(summary.p95Final, isMoney)}
                  </b>
                  ; 5th pct final:{' '}
                  <b className={summary.p5Final >= 0 ? 'pos' : 'neg'}>
                    {fmtValue(summary.p5Final, isMoney)}
                  </b>.
                </span>
              </li>
              <li>
                <span>
                  The deepest 5% of runs had max drawdown of{' '}
                  <b className="neg">{fmtValue(summary.maxDD_p5, isMoney)}</b>{' '}
                  or worse
                  {hasDdPct && (
                    <>
                      {' '}
                      (<b className="neg">{summary.maxDDPct_p5.toFixed(1)}%</b> of capital)
                    </>
                  )}.
                </span>
              </li>
              {ddBreachPct > 0 && (
                <li>
                  <span>
                    <b className="neg">{ddBreachPct.toFixed(1)}%</b> of runs
                    reached or exceeded the configured drawdown threshold of{' '}
                    <b>{fmtValue(ddThreshold, isMoney)}</b>.
                  </span>
                </li>
              )}
              <li><span>95th pct longest loss streak: <b className="amber">{fmtTrades(summary.lossStreak_p95)} trades</b> in a row.</span></li>
            </>
          ) : (
            <>
              <li><span>Total P&amp;L is invariant across reshuffles: <b>{fmtValue(summary.medianFinal, isMoney)}</b>. Only the path changes.</span></li>
              <li>
                <span>
                  The deepest 5% of reshuffles had max drawdown of{' '}
                  <b className="neg">{fmtValue(summary.maxDD_p5, isMoney)}</b>{' '}
                  or worse
                  {hasDdPct && (
                    <>
              ...
              <li><span>Median max drawdown across reshuffles: <b className="amber">{fmtValue(summary.maxDD_p50, isMoney)}</b>.</span></li>
              {ddBreachPct > 0 && (
                <li>
                  <span>
                    <b className="neg">{ddBreachPct.toFixed(1)}%</b> of reshuffles
                    reached or exceeded the configured drawdown threshold of{' '}
                    <b>{fmtValue(ddThreshold, isMoney)}</b>.
                  </span>
                </li>
        ...
        <div className="sim-table-scroll">
          <table className="sim-table">
            ...
            <tbody>
              ...
              <tr>
                <td>Trade Sharpe <span className="faint">(unannualized)</span></td>
              ...
              <tr>
                ...
                <td className="faint">—</td>
                <td className="faint">—</td>
                <td>{Number.isFinite(summary.pf_p50) ? fmtRatio(summary.pf_p50) : '∞'}</td>
                <td className="faint">—</td>
                <td className="faint">—</td>
              </tr>
              {summary.infinitePfCount > 0 && (
                <tr>
                  <td>Runs with ∞ PF</td>
                  <td className="faint">—</td>
                  <td className="faint">—</td>
                  <td className="amber">{summary.infinitePfCount}</td>
                  <td className="faint">—</td>
                  <td className="faint">—</td>
                </tr>
              )}
...
/*  Bands                                                              */
/* ------------------------------------------------------------------ */
function BandsPanel({ result, actualCurve, isMoney, resultIsStale }) {
  const sampledShown = result.sampledShown ?? 0;
  const noteText = sampledShown > 0 && sampledShown < result.runs
    ? `showing ${sampledShown.toLocaleString()} of ${result.runs.toLocaleString()} paths`
    : `${result.runs.toLocaleString()} paths`;

  return (
    <Panel title="Cumulative Equity Bands" note={noteText}>
      <div className="sim-chart-wrap">
        <FanChart result={result} actualCurve={actualCurve} isMoney={isMoney} showActual={!resultIsStale} />
      </div>
      <div className="sim-legend">
        <span className="sim-legend-item">
          <span className="sim-legend-line" style={{ background: '#F59E0B' }} />Median path
        </span>
        <span className="sim-legend-item">
          <span className="sim-legend-band" style={{ background: 'rgba(245,158,11,.18)' }} />25–75% band
        </span>
        <span className="sim-legend-item">
          <span className="sim-legend-band" style={{ background: 'rgba(245,158,11,.08)' }} />5–95% band
        </span>
        <span className="sim-legend-item">
          <span className="sim-legend-line" style={{ background: '#4C8BF5' }} />Your actual path
        </span>
        {sampledShown > 0 && (
          <span className="sim-legend-item">
            <span className="sim-legend-line" style={{ background: 'rgba(245,158,11,.4)' }} />Individual paths
          </span>
        )}
      </div>
      <p className="sim-foot">
        {result.method === 'permutation'
          ? 'Every reshuffle lands on the same final total — only the path bends.'
          : result.method === 'block'
            ? `Sampled in blocks of ${result.blockSize} consecutive trades to preserve local streak clustering.`
            : 'Sampled independently with replacement. Bands show the range of outcomes a fresh sample of the same size would produce.'}
      </p>
    </Panel>
  );
}
...
function DrawdownPanel({
  ...
  return (
    ...
    <Panel
      title="Drawdown Path"
      note="Current equity minus running peak"
    >
      <div className="sim-chart-wrap">
        <FanChart
          result={result}
      ...
      <p className="sim-foot">
        ...
        <b> 0</b> means the path is at a peak or has fully recovered.
        Negative values indicate the depth below the peak.
        {result.ddThreshold != null && (
          <>
            {' '}The red line marks the configured
            <b> drawdown threshold</b>.
          </>
        )}
      </p>
    </Panel>
    ...
    <div className="sim-grid-2">
      <Panel title="Max Drawdown Distribution" note={`Worst DD depth per run · ${result.runs} runs`}>
        <div className="sim-chart-sm">
          <DistributionChart values={result.maxDDs} color="#ef4444" height={320} />
        </div>
        <p className="sim-foot">
          Deepest 5% of runs: <b style={{ color: '#f87171' }}>
            {fmtValue(summary.maxDD_p5, isMoney)}
          </b>
          {hasDdPct && (
            <>
              {' '}· <b style={{ color: '#f87171' }}>
                {summary.maxDDPct_p5.toFixed(1)}%
              </b> of capital
            </>
          )}
          <br />
          ...
          {hasDdPct && (
            <> · <b>{summary.maxDDPct_p50.toFixed(1)}%</b></>
          )}
        </p>
      </Panel>

      <Panel title="Drawdown Duration" note="Peak → recovery (trades)">
        <div className="sim-chart-sm">
          <DistributionChart
            values={result.recoveryTrades.filter(v => v >= 0)}
            color="#F59E0B"
            height={320}
            tooltipSuffix="trades peak→recovery"
          />
        </div>
        <p className="sim-foot">
...
function RatiosPanel({ result }) {
    ...
    return (
      <Panel>
        <div className="sim-na">
          ...
          <span>
            Ratio distributions require <b>Bootstrap</b> or <b>Block Bootstrap</b> mode.
          </span>

          <span>
            Permutation keeps the exact same trades and only changes their order,
            so trade-level Sharpe, Sortino, Profit Factor, Expectancy, and Win Rate
            do not form meaningful cross-run distributions.
          </span>
        </div>
      </Panel>
    );
  ...
  const items = [
    { title: 'Trade Sharpe (unannualized)', values: result.sharpes, color: '#F59E0B' },
    { title: 'Trade Sortino (unannualized)', values: result.sortinos, color: '#F59E0B' },
    { title: 'Profit Factor', values: result.profitFactors, color: '#22c55e' },
    { title: 'Expectancy', values: result.expectancies, color: '#22c55e' },
    { title: 'Win Rate (%)', values: result.winRates.map(v => v * 100), color: '#F59E0B' },
  ];
  const infinitePfCount = result.infinitePfCount || 0;

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      {infinitePfCount > 0 && (
        <Panel title="Profit Factor Note">
          <p className="sim-foot" style={{ marginTop: 0 }}>
            <b>{infinitePfCount.toLocaleString()}</b> runs had no losing trades,
            so their Profit Factor is <b>∞</b>. Those runs are excluded from the
            finite Profit Factor distribution.
          </p>
        </Panel>
      )}
    ...
    <div className="sim-grid-2">
      <Panel title="Longest Win Streak" note={`across ${result.runs} runs`}>
        ...
        <p className="sim-foot">
          ...
          <br /><span style={{ opacity: .7 }}>Breakeven (0R) trades reset both streaks.</span>
        </p>
      </Panel>
...
function TargetsPanel({ result, isMoney }) {
  ...
  const isPermutation = result.method === 'permutation';
  return (
    <Panel title="Time to Reach Target" note={isMoney ? 'dollar targets' : 'R-multiple targets'}>
      <div className="sim-table-scroll">
        <table className="sim-table">
          <thead>
            <tr>
          ...
          <tbody>
            {result.timeToTarget.map(t => (
              <tr key={t.target}>
                ...
                <td className={t.finalPct   >= 50 ? 'pos' : 'neg'}>{t.finalPct.toFixed(1)}%</td>
        ...
        {isPermutation && (
          <p className="sim-foot">
            In permutation mode, <b>P(final ≥ target)</b> is identical across every
            reshuffle because the total P&amp;L is invariant. The useful sequence-risk
            signal is whether the target was <b>touched along the path</b> and how many
            trades it took to get there.
          </p>
        )}
      </div>
      <p className="sim-foot">
        <b>P(ever touched)</b> = fraction of runs that touched the target at any point.
        <b> P(final ≥ target)</b> = fraction that finished at or above it.
        Median trades = 50th percentile of first-hit trade counts
        <b> among runs that reached the target</b>.
      </p>
    </Panel>
  );
}
  ...
  return (
    <>
      ...
      <div className="sim-panels">
        ...
        {tab === 'overview' && (
          <OverviewPanel result={result} summary={summary} isMoney={isMoney} initialCapital={initialCapital} dollarsPerR={dollarsPerR} />
        )}
        {tab === 'bands' && (
          <BandsPanel
            result={result}
