...
export default function KPIGrid() {
  ...
  if (!stats || stats.n === 0) {
    return (
      <div style={{ backgroundColor: '#11151F', border: '1px solid #212836', borderRadius: '14px', padding: '48px 24px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', fontFamily: "'Inter', sans-serif", boxShadow: '0 4px 20px rgba(0, 0, 0, 0.2)' }}>
        <div style={{ width: '48px', height: '48px', borderRadius: '12px', backgroundColor: '#161B26', border: '1px solid #212836', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '16px' }}>
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#545E6E" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <rect x="3" y="3" width="18" height="18" rx="2" ry="2" />
            <line x1="3" y1="9" x2="21" y2="9" />
            <line x1="9" y1="21" x2="9" y2="9" />
          </svg>
        </div>
        <h4 style={{ margin: '0 0 6px 0', fontSize: '15px', fontWeight: '600', color: '#E7E9EE' }}>No Trading Data Available</h4>
        <p style={{ margin: 0, fontSize: '12px', color: '#8892A3', maxWidth: '320px', lineHeight: '1.5' }}>
  ...
  const kpis = isMoney
    ? [
        ...
        { label: 'Avg Win', value: formatMoney(stats.avgWin), cls: 'pos', sub: 'per winning trade' },
        { label: 'Avg Loss', value: formatMoney(stats.avgLoss), cls: 'neg', sub: 'per losing trade' },
        { label: 'Max Drawdown', value: stats.maxDD != null ? formatMoney(stats.maxDD) : '—', cls: 'neg', sub: 'peak to trough' },
        ...
        { label: 'Best Trade', value: stats.bestTrade ? formatMoney(stats.bestTrade.score) : '—', cls: 'pos', sub: stats.bestTrade ? `${stats.bestTrade.date} ${stats.bestTrade.entry}` : '' },
        { label: 'Worst Trade', value: stats.worstTrade ? formatMoney(stats.worstTrade.score) : '—', cls: 'neg', sub: stats.worstTrade ? `${stats.worstTrade.date} ${stats.worstTrade.entry}` : '' },
      ]
    : [
        { label: 'Win Rate', value: formatPct(stats.winRate), cls: stats.winRate >= 50 ? 'pos' : 'neg', sub: `${stats.wins?.length ?? 0}W` },
        { label: 'Loss Rate', value: formatPct(stats.lossRate), cls: 'neg', sub: `${stats.losses?.length ?? 0}L` },
